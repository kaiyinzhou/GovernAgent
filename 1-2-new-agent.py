...
# experience_add = json.load(open("./configs/new_experiences.json", encoding='utf-8'))
    ...
    def _function_agent_field(self, raw_content, answers, mode):
        ...
        for answer in answers:
            ...
            for key, value in answer.items():
                ...
                elif "动作" in key and "拷贝" in value:
                    try:
...
病历文本：
{}"""
        PROMPT_agent_experience = """你是一个病历章节拆分智能体，负责将杂乱的病历文书拆分成独立的章节。
给你一份病历文书，其中可能包含一个或多个病历章节，请根据病历书写规范从下面的病历文书中抽取“{}”章节内容：{}。
        ...
        for attempt in range(max_iters + 1):
            ...
            if batch_query:
                ...
                for task, predict, raw_content in zip(tasks, batch_predicts, batch_raw_content):
                    ...
                    print(predict)
                    print(part_sections)
                    part_sections = [ps.replace("未抽取到相关内容", "") for ps in part_sections if
                                     ps.replace("未抽取到相关内容", "")]
                    # 同一个类型的章节可以识别出多个结果。
                    final_content = []
...
{}"""
        ...
        PROMPT_agent_experience = """你是一个病历字段抽取智能体，请完成下面的任务。
请从下面的“{}”病历中抽取“{}”字段的内容， “{}”是“{}”。
...
if __name__ == "__main__":
    ...
    if "agent" in model_name:
        ...
        os.makedirs("./predicts/{}-测试/".format(model_name), exist_ok=True)
        extract_results = loop.run_until_complete(extract.batch_inference(datas, max_iters))
        write_results_to_file(extract_results, "./predicts/{}-测试/{}_results_wo_base.json".format(model_name, data_ctype, max_iters))

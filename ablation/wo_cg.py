...
class PiplineInference:
    ...
    async def section_inference(self, datas):
        ...
        :return: 推理结果列表
        """
        PROMPT_extract = """你是一个病历章节拆分智能体，负责将杂乱的病历文书拆分成独立的章节。
给你一份病历文书，其中可能包含一个或多个病历章节，请根据病历书写规范从下面的病历文书中抽取“{}”章节内容，{}。

抽取时请注意以下事项：
（1）抽取的章节内容必须与原文一致，并且在原文中连续出现。
（2）抽取时不需要对病历文书中的乱码进行处理，但如果该章节出现多次且内容重复，仅需要抽取最完整的一份章节。
（3）按照下面的格式返回拆分结果：```json[{{"章节类型": "_", "章节内容": "_"}}]```。

病历文本：
{}"""
        PROMPT_agent = """你是一个病历章节拆分智能体，负责将杂乱的病历文书拆分成独立的章节。
给你一份病历文书，其中可能包含一个或多个病历章节，请根据病历书写规范从下面的病历文书中抽取“{}”章节内容，{}。

抽取时请注意以下事项：
（1）通过使用“拷贝”和“生成”动作完成章节内容抽取。
其中，“拷贝”动作指的是你需要发出“拷贝命令”，并同时给出“拷贝起始字符”、“拷贝终止字符”、“拷贝长度”参数，从而实现部分章节内容的抽取。
其中拷贝起始字符是章节的开始标志，拷贝终止字符是章节的结束标志，拷贝起始位置和终止位置通常包含5-8个字符。拷贝长度是从拷贝起始位置到终止位置的字符长度。
当需要跳过某些内容时需要在跳过位置停止拷贝，然后重新寻找拷贝起始位置，并发出“拷贝命令”，并给出拷贝参数。
当需要拷贝的字段内容较短时（小于20 tokens），需要使用“生成”动作，即你需要发出“生成命令”，并直接生成目标内容，从而实现章节内容的抽取。不允许重复拷贝或生成相同内容。
...
（3）当需要抽取的章节内容在病历文本中不存在时，返回```json[{{"动作1": "生成", "生成内容": "未抽取到相关内容"}}]```。
...
{}"""
        ...
        for data in datas:
            data["sub_contents"] = []
            for section_name in section_explain:  # 每一个样本，所有的章节都得分析一遍
                sub_meta = copy.deepcopy(data["meta"])
                sub_meta["sec_name"] = section_name
                raw_content = data["content"]
                query = PROMPT.format(section_name, section_explain[section_name],
                                      raw_content.replace("<sep1>", ":").replace("<sep2>", " "))
                batch_query.append(query)
                batch_meta.append(sub_meta)
                batch_datas.append(data)
                batch_raw_content.append(raw_content.replace("<sep1>", ":").replace("<sep2>", " "))
        batch_predicts = await async_process_queries(batch_query, self.url, self.model_name)

        for data, part_sections, meta, raw_content in zip(batch_datas, batch_predicts, batch_meta, batch_raw_content):
            if part_sections:


                if self.ctype == "extract":
                    part_sections = self._function_field_extract(part_sections)
                elif self.ctype == "agent":
                    part_sections = self._function_agent_field(raw_content, part_sections)
                else:
                    print("ERROR")
                    return

                for sub_content in part_sections:  # 每一个拷贝的内容是一个单独的章节
                    # if sub_content.replace("未抽取到相关内容", ""):
                    data["sub_contents"].append({"sub_content": sub_content.replace("未抽取到相关内容", ""),
                                                     "sub_meta": meta})
            # if not data["sub_contents"]:
            #     data["sub_contents"].append({"sub_content": raw_content.replace("<sep1>", ":").replace("<sep2>", " "),
            #                                  "sub_meta": raw_meta})
...
if __name__ == "__main__":
    ...
    # model_name = "agent-7b"
    model_name = "extract-14b-wo-cg"
    # model_name = "extract-llama-sft-8b"
    data_ctype = "test"
    # model_name = "goveragent-7b"
    # data_ctype = "test"

    url = "http://10.0.0.230:8899/v1"

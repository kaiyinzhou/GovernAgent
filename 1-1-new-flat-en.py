        ...
        try:
            ...
            for answer in answers:
                if "章节内容" in answer:
                    ...
                    sub_content = answer["field_content"]
...
# 请从下面的“{}”病历中抽取“{}”字段的内容，“{}”是“{}”。
# 抽取时请注意以下事项：
# （1）当field_content重复出现时，只需抽取一次。
# （2）对于病历中的乱码部分，不需要抽取。
# （3）当需要抽取的内容并非连续出现时，请确保抽取内容完整，不连续的内容用“##”号连接。保留关键词，例如“性别”、“年龄”等。
# （4）按照下面的格式进行抽取：```json[{{"field_name": "_", "field_content": "_"}}]```。
# （5）当需要抽取的字段在病历文本中不存在时，“field_content”部分填写“Not extracted relevant content”。
#
# 病历文本：
# {}"""
...
(5) If the required field does not exist in the medical record text, fill in "Not extracted relevant content" (content not found) in the "field_content" section.

...
def compute_subcontent_metrics(golds: List[Dict], predicts: List[Dict], similarity_threshold=0.8):
    ...
    for gold, pred in zip(golds, predicts):
        ...
        gold_subs = gold["sub_contents"]
        pred_subs = pred["sub_contents"]

        tp, fp, fn = 0, 0, 0

        for i, g_sub in enumerate(gold_subs):
            g_sec_name = g_sub["sub_meta"]["sec_name"]
            g_content = g_sub["sub_content"]

            best_j = -1
            best_sim = 0.0

            for j, p_sub in enumerate(pred_subs):
                p_sec_name = p_sub["sub_meta"]["sec_name"]
                p_content = p_sub["sub_content"]
...
def compute_fields_metrics(golds: List[Dict], predicts: List[Dict], similarity_threshold=0.8):
    ...
    for gold, pred in zip(golds, predicts):
        ...
        gold_fields = [gf for gf in gold_fields if not (gf[0] == '' and gf[1] == '')]
        pred_fields = [pf for pf in pred_fields if not (pf[0] == '' and pf[1] == '')]
...
if __name__ == "__main__":
    ...
    for file_ctype in ["mimic"]:
        ...
        hos_gold_datas, hos_predict_datas = rebuild_datas(
            ...
            # read_jsonl("./predicts/{}/test_results_0.json".format(model_name, file_ctype))
        ...
        for hos_name, gold_datas in hos_gold_datas.items():
            ...
            subcontent_metrics = compute_average(
                compute_subcontent_metrics(gold_datas, predict_datas, similarity_threshold=0.4))
            fields_metrics = compute_average(
                compute_fields_metrics(gold_datas, predict_datas, similarity_threshold=0.4))
